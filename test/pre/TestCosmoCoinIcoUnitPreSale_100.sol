...
contract TestCosmoCoinIcoUnitPreSale_100 {
    ...
    function testPreSaleBonus100() public {
        uint256 expectedBalance = 0;
        uint256 balance = 0;

        // Presale
        cosmoCoinIco.startPreSale();
        cosmoCoinIco.pausePreSale();
        cosmoCoinIco.restartPreSale();

        // 100 finney
        cosmoCoinIco.call.value(100 finney)();
        expectedBalance = expectedBalance + 1100000 finney;
        balance = cosmoCoinIco.cosmoCoin().balanceOf(this);
        Assert.equal(balance, expectedBalance, "");

        cosmoCoinIco.endPreSale();
        Assert.equal(ETH_FUND_WALLET.balance, 100 finney, "final balance");
    }
}
